...
export function Toolbar() {
  ...
  return (
    <div className="toolbar">
      ...
      <button
        onClick={() => {
          setMsg('Нормализация запущена…');
          const r = normalizeAll();
          setMsg(r.done > 0 ? `Нормализовано моделей: ${r.done}` : 'Все модели уже нормализованы.');
        }}
      ...
      <button
        ...
        title="Подобрать класс/подкласс по классификатору (по ключевым словам в коде модели и в иерархии). Уверенные совпадения проставляются автоматически, остальные — как подсказки в карточке модели."
      >
        Классифицировать
      </button>
      <button
        ...
        title={
          !getApiKey()
            ? 'Подключите OpenAI ключ кнопкой «Настройки» в шапке'
      ...
      <button
        title="Заполнить характеристики из встроенной базы 35 000+ моделей оборудования (без обращения к ИИ). Поиск идёт сначала по базе, затем при неудаче — пользователь может дополнить из интернета. Существующие зафиксированные значения не трогаются."
        onClick={async () => {
          const state = useStore.getState();
          const update = state.updateModel;
          ...
          setMsg(`База моделей: загрузка… (${need.length} моделей)`);
          let db;
          try {
      ...
      <span className="sep" />
      <button
        title="Экспорт в XLSX: листы «Иерархия», «Классификация», «Характеристики», «ВВ», «Техкарты»."
        onClick={() => {
          setMsg('Формирование файла…');
          const s = useStore.getState();
          const stamp = new Date().toISOString().slice(0, 10);
          downloadWorkbook(s.hierarchy, s.models, `nsi-${stamp}.xlsx`);
          setMsg('Файл экспорта скачан. Окно остаётся открытым.');
        }}
      >
        Экспорт
      </button>

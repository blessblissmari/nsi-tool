...
export interface TechCardRow {
  ...
  source: SourceKind;
  /**
   * Источник по каждой ячейке отдельно (созвон 28.04.2026: «раскраска тех карты
   * по столбцам ИИ vs человек»). Заполняется ИИ-методами для каждого поля,
   * которое они изменили; для полей, которые редактировал эксперт вручную,
   * хранится `manual`. Если для конкретного поля записи нет — берём общий
   * `source` строки.
   */
  cellSources?: Record<string, SourceKind>;
  /**
   * URL источника по ячейке (когда поле взято из открытых источников).
   * Используется для бейджа 🔗 в шапке столбца «🌐 Поиск в интернете».
   */

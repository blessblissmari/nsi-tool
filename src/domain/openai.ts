...
export const openaiAiProvider: AiProvider = {
  async classify(input: {
    ...
    const system =
      'Ты эксперт по классификации промышленного оборудования. ' +
      'Тебе дан НОРМАЛИЗОВАННЫЙ КОД МОДЕЛИ оборудования — это маркировка ' +
      'производителя (буквы, цифры, дефисы). ' +
      'Твоя задача — определить к какому классу и подклассу из СПИСКА ниже ' +
      'относится это оборудование. ' +
      'АЛГОРИТМ: ' +
      '(1) Проанализируй код модели — определи производителя и тип оборудования ' +
      'по общедоступным каталогам (например: Д160 = насос, GA37 = компрессор Atlas Copco, ' +
      'MML2550 = токарный станок, FMA = фильтр, K37 = редуктор SEW). ' +
      '(2) Сопоставь тип оборудования с одним из классов/подклассов из списка. ' +
      '(3) Если код тебе незнаком — используй любые подсказки из документа или ' +
      'структурные паттерны кода. ' +
      '(4) confidence ≥ 0.85 если ты уверен в производителе/типе, ' +
      '0.6-0.84 если догадка по паттерну, < 0.6 если не уверен. ' +
      'Возвращай ТОЛЬКО json: ' +
      '{"items":[{"className":"...","subclassName":"...","confidence":0.0-1.0,"reason":"..."}]}. ' +
      'Используй ТОЛЬКО классы и подклассы из списка. Не более 3 кандидатов. ' +
      'В reason укажи: какой это производитель/тип оборудования и почему выбран этот класс.';
    const user = `Нормализованный код модели: ${code}

Доступные классы и подклассы (выбирай ТОЛЬКО из них):
${classOptions.map(c => `• ${c.name}: ${c.subclasses.join(', ')}`).join('\n')}
${docSnippet ? `\nФрагмент документа модели:\n${docSnippet}` : ''}`;
